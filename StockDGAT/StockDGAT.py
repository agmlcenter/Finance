...
def get_acc_for_a_day(p):
    ...
    key = 'Adj Close'
    all_key = ['Adj Close', 'High', 'Low', 'Open', 'Close', 'Volume']
    ...
    for stock in list(dict_of_data.keys()):
        ...
        for keys in all_key:
            ...
            if len(time_series) < max_length:
                df[keys] = ((max_length - len(time_series)) * [time_series[0]] + time_series)[-period:]
            else:
                df[keys] = time_series[-period:]
        dict_of_data[stock] = df
    features, labels = prepare_graph_data(dict_of_data, 'Adj Close', num_days=period, per=4)
    ...
    train_data = data_list[:-64]
    test_data = data_list[-64:]

    # Initialize model
    model = GATSequence(in_channels=3, out_channels=4, num_graphs=num_graph).to(
    ...
    with torch.no_grad():
        for data in loader.DataLoader(test_data, batch_size=32):
