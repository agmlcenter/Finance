...
def get_acc_for_a_day(p):
    ...
    for stock in list(dict_of_data.keys()):
        ...
        if len(time_series) < max_length:
            ...
            df[key] = time_series
        dict_of_data[stock] = df.tail(20 + index).head(22)
...
if __name__ == '__main__':
    ...
    # Construct tuples for multiprocessing with 10 different seeds
    seeds = list(range(num_of_run))

    for idx in range(64):
        for j in range(num_of_run):
            all_tuple.append((65 - idx, dict_of_data, seeds[j]))
    ...
    Average_Acc = 0
    for j in range(num_of_run):
        Average_Acc += np.average([results[j + i * num_of_run] for i in range(64)]) / 50
